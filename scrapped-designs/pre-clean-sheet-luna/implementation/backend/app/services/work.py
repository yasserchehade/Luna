...
def prepare_approval_required_work(
    cursor: Cursor[dict[str, Any]],
    *,
    workspace_id: object,
    request: WorkOrderCreate,
    approval_reason: str,
    requested_approver_role: HouseholdRole = HouseholdRole.owner,
) -> WorkOrderWithApproval:
    work_order = create_work_order(
        cursor,
        workspace_id=workspace_id,
    ...
    record_audit_event(
        ...
        metadata={
            "work_type": request.work_type,
            "capability": request.capability_required.value,
            "decision": AuthorityDecision.approval_required.value,
            "approver_role": requested_approver_role.value,
            "policy_id": None,
            "reason": "This work changes household cabinet path or storage state.",
        },
    )
    approval = request_approval(
        cursor,
