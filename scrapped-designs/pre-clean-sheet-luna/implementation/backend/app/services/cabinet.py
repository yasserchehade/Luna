...
from app.models.work import Capability, HouseholdRole, WorkOrderCreate
from app.services.audit import record_audit_event
from app.services.work import prepare_approval_required_work
...
def plan_document_cabinet_path(document_id: str) -> dict[str, object]:
    ...
    return {
        ...
        "document_classification": _document_classification(bill),
        "reasons": reasons,
    }
...
def confirm_document_cabinet_path_with_cursor(
...
def mark_document_cabinet_needs_review_with_cursor(
    cursor: Cursor[dict[str, Any]],
    *,
    document_id: str,
) -> None:
    cursor.execute(
        """
        UPDATE documents
        SET cabinet_status = 'needs_review'
        WHERE id = %s
        """,
        (document_id,),
    )
...
def prepare_document_cabinet_plan_work(
    cursor: Cursor[dict[str, Any]],
    *,
    workspace_id: object,
    ...
    suggested_path = str(plan["suggested_cabinet_path"])
    reasons = [str(reason) for reason in plan["reasons"]]
    classification = str(plan.get("document_classification") or "household_document")
    ...
    return prepare_approval_required_work(
        cursor,
        workspace_id=workspace_id,
        request=WorkOrderCreate(
            ...
            description=(
                "Luna classified the document and prepared a household cabinet path. "
                "The original document remains unchanged until approval."
            ),
            ...
            evidence={
                ...
                "source_of_truth": "original_document",
            },
        ),
        approval_reason=(
            "Luna prepared a filing suggestion. Approve to confirm the cabinet path, "
            "or reject/dismiss to leave the original document unchanged."
        ),
...
def _document_classification(bill: dict[str, object] | None) -> str:
    if bill is not None:
        return "bill"
