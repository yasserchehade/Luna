...
from app.services.cabinet import prepare_document_cabinet_plan_work, save_document_cabinet_plan
from app.services.extraction import get_extractor
from app.services.obligations import sync_bill_obligation
...
def ingest_bill(request: BillIngestRequest) -> BillIngestResponse:
    ...
    with get_connection() as connection:
        with connection.cursor() as cursor:
            prepare_document_cabinet_plan_work(
                cursor,
                workspace_id=document["workspace_id"],
                plan=cabinet_plan,
            )
    ...
    with get_connection() as connection:
        with connection.cursor() as cursor:
            ...
            if row is not None:
                ...
                if row["review_status"] == BillReviewStatus.confirmed.value:
