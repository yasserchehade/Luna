...
from app.services.obligations import refresh_overdue_obligations_for_workbench_read
...
def _obligation_from_row(row) -> Obligation:
    return Obligation(
        id=str(row["id"]),
...
def household_summary() -> HouseholdSummary:
    with get_connection() as connection:
        workspace_id = get_default_workspace_id(connection)
        with connection.cursor() as cursor:
            refresh_overdue_obligations_for_workbench_read(
                cursor,
                workspace_id=workspace_id,
            )
            cursor.execute(
            ...
            upcoming_obligation_rows = cursor.fetchall()
            cursor.execute(
                """
                SELECT
                    id,
            ...
            overdue_obligation_rows = cursor.fetchall()
            cursor.execute(
                """
                SELECT
                    id,
            ...
            needs_review_obligation_rows = cursor.fetchall()
    ...
    return HouseholdSummary(
        ...
        upcoming_obligations=[
            _obligation_from_row(row)
            for row in upcoming_obligation_rows
        ],
        overdue_obligations=[
            _obligation_from_row(row)
        ...
        needs_review_obligations=[
            _obligation_from_row(row)
            for row in needs_review_obligation_rows
        ],
    )
