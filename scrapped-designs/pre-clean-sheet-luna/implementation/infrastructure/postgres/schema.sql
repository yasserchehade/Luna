...
CREATE TABLE IF NOT EXISTS obligations (
    ...
    status TEXT NOT NULL DEFAULT 'needs_review' CHECK (
        status IN ('needs_review', 'upcoming', 'due_soon', 'overdue', 'paid', 'archived')
    ),
    evidence JSONB NOT NULL DEFAULT '{}'::jsonb,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (workspace_id, source_bill_id)
);
...
ALTER TABLE obligations
    ADD COLUMN IF NOT EXISTS currency CHAR(3) NOT NULL DEFAULT 'AUD';
...
ALTER TABLE obligations
    ...
    CHECK (status IN ('needs_review', 'upcoming', 'due_soon', 'overdue', 'paid', 'archived'));
...
CREATE INDEX IF NOT EXISTS idx_obligations_workspace_status
    ON obligations(workspace_id, status, due_date);
CREATE UNIQUE INDEX IF NOT EXISTS idx_obligations_bill
    ON obligations(workspace_id, source_bill_id);
