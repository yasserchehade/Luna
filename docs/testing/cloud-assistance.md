...
| Managed device provisioning | `desktop/server-contract/managedIntelligenceProvisioning.test.ts` proves device proof reaches only narrow credential generation, while `liteLlmManagedAccessClient.test.ts` proves exact route/model, expiry, rate/token/spend caps, opaque attribution and alias revocation |
