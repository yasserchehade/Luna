...
| Unreadable Consent history cannot disable verified access | `verified_consent_lookup_skips_unreadable_history_without_reusing_it` excludes unreadable grants from authorization and portable export while returning verified current grants with a separate warning flag; `verified current permissions remain visible beside an unreadable-history warning` proves Options displays the real permission state, and a post-commit portable-memory delay is reported as pending synchronisation rather than a failed permission |
...
| Stale default recovery | `a stale saved default proposes the configured replacement route` proves an unavailable saved model cannot trap the default controls in a disabled state. Luna proposes a configured replacement but still requires the Household Organiser to choose **Save default**, so permissions never transfer silently. |
