...
| Stale access after cancellation or device revocation | Access loss queues the device-attributed LiteLLM alias for deletion. Server reconciliation retries deletion, and authenticated desktop synchronisation removes the local OS-vault credential whenever the Household is no longer entitled. Virtual keys also expire after 24 hours and carry rate, token and spend caps. |
