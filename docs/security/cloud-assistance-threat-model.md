...
| Stale access after cancellation or device revocation | The server reserves the deterministic device alias while rechecking eligibility before key creation, so a concurrent entitlement loss prevents minting or leaves a durable revocation handle. Access loss queues that alias for deletion, and a re-grant cannot erase it before deletion. Server reconciliation retries deletion, and authenticated desktop synchronisation removes the local OS-vault credential whenever the Household is no longer entitled. Virtual-key configuration is rejected above 24 hours and keys renew before expiry; rate/token caps apply to keys and one shared LiteLLM team spend ceiling applies to the whole Household entitlement scope. |
