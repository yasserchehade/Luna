...
| Stale access after cancellation or device revocation | Access loss queues the device-attributed LiteLLM alias for deletion. A re-grant cannot erase that handle before deletion. Server reconciliation retries deletion, and authenticated desktop synchronisation removes the local OS-vault credential whenever the Household is no longer entitled. Virtual keys expire after 24 hours and renew before expiry; rate/token caps apply to keys and one shared LiteLLM team spend ceiling applies to the whole Household entitlement scope. |
