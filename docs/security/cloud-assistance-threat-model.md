...
| Stale access after cancellation or device revocation | The server reserves the deterministic device alias while rechecking eligibility, budget scope and cap before key creation, so a concurrent entitlement loss prevents minting or leaves a durable revocation handle. A short provisioning lease prevents reconciliation from clearing that handle before an in-flight mint completes. Access loss queues the alias for deletion, and a re-grant cannot erase it before deletion. Server reconciliation retries deletion, and authenticated desktop synchronisation removes the local OS-vault credential whenever the Household is no longer entitled. Requested and returned virtual-key lifetimes are rejected above 24 hours; rate/token caps apply to keys and one shared LiteLLM team spend ceiling applies to the whole Household entitlement scope. |
