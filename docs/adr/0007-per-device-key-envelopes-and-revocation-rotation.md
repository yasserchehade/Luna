...
Routine return on an already trusted device keeps the MFA-verified account session in the operating-system credential vault and asks only for the local Device PIN. **Lock Luna** clears the in-memory Household-key unlock while retaining that account session. Explicit sign-out removes the account session and therefore requires email, password, authenticator MFA and the Device PIN on the next visit. New devices and recovery continue to require full account authentication.

Luna stores only public keys and encrypted age envelopes: one current-key envelope per active device and a separate historical keyring envelope protected by the offline Recovery Key. The keyring retains each Household key epoch so a newly recovered Trusted Device can rebuild older signed History; revoked devices receive no future epoch key. Replacement recovery is two-phase: local material remains pending until the service accepts the device at the current key epoch. Account-password recovery therefore cannot decrypt Household memory or accidentally complete Trusted Device recovery.
