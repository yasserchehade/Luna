...
The founder Trusted Device successfully provisioned its narrow gateway credential after the administration URL and rotated Cloudflare Service Auth identifier were corrected in Supabase. The reconciliation endpoint then cleared the failed alias with zero failures, and the desktop reported only the independent portable-memory warning. This proves installed-device provisioning and vault storage. Issue #38 now implements the consent-gated ordinary-message path on the hardened gateway in a review branch; a real managed reply and entitlement revocation still require explicit release-gate evidence before the remote gate closes. The canary account is verified with authenticator MFA and has its Household entitlement, but intentionally has no Trusted Device until the isolated live canary enrolls one.
