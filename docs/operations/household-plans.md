...
After the next unlock, an entitled enrolled Trusted Device requests a five-minute challenge, signs it with its existing device-authorisation key and asks the provisioning function for a narrow LiteLLM virtual key whose lifetime cannot exceed 24 hours. Before minting, the server reserves the deterministic device alias while atomically rechecking that the Household entitlement and Trusted Device remain eligible. The desktop retains the returned key only in its operating-system credential vault.
